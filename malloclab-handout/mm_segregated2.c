...
#define LIST_END    NULL /* Front and end of linked lists */
...
{ 
    ...
    int error_flag = 0;
    int error_check = 0; /* set to 0 if called outside malloc() and free() */
    int silent = 0;
    ...
    if (error_flag || !silent) {
        ...
        for (bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp)) {
           ...
           if (GET_ALLOC(HDRP(bp))) {
               printf("%c|%15p|%7zu|%7zu|%2s|%18s|%18s|%c|%c\n",
                   alloc_char,
                   bp,
                   size,
                   payload,
               ...
               printf("%c|%15p|%7zu|%7zu|%2d|%18p|%18p|%c|%c\n",
                   alloc_char,
                   bp,
                   size,
                   payload,
